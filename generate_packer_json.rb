...
class PackerJsonGenerator
  ...
  def generate_vagrant_file(options, output_dir)
    erb = VagrantFile.new(@basedir,options['vm_description'],simpconfig['cli::network::ipaddress'],options['mac_address'],options['host_only_network_name'])
    vfile_contents=erb.render

    File.open("#{output_dir}/Vagrantfile",'w') do |h|
      h.write(vfile_contents)
      h.close
    end
  end

  def get_packer_json(json_file)
    if File.file?(json_file)
      f = File.open(json_file,'r')
      json = String.new
      # remove all the explanatory comments in the json file,
      ...
      raise "JSON file #{json_file} does not exist or is not a file."
    end
  end
  ...
  def validate_settings(settings)
    validated = settings.dup
    case settings['firmware']
    when 'bios','efi'
      validated['firmware'] = settings['firmware']
    else
      $stderr.puts "WARNING: Unknown firmware option #{settings['firmware'}."
      $stderr.puts "         Defaulting to 'bios'."
    ...
    when /[Nn][Oo]?/,'false',false
      ...
      $stderr.puts "WARNING: Invalid Headless option #{settings['headless'}."
      $stderr.puts "         Defaulting to 'true'."
      validated['headless'] = 'true'
    end
    ...
    when ''
      ...
      $stderr.puts "WARNING: Invalid disk_encrypt option #{settings['disk_encrypt'}."
      $stderr.puts "         Defaulting to ''."
      validated['disk_encrypt'] = ''
    end
...
# Network does not exists, create it and return the name
    ...
    if ( newnet.include? "was successfully created" )
      ...
      puts "Creation of network unsuccessful. #{newnet}"
    end
    return nil
  end
  ...
  def parse_command_line(args)
    @workingdir = argv[0]
    @testdir = argv[1]
    @box_type = argv[2].nil? ? 'simp' : argv[2]
  end

  def run(args)
    parse_command_line(args)

    #FIXME move json templates into template dir
    json_tmp = @basedir + "/#{@box_type}.json.template"
...
json = get_packer_json(json_tmp)
File.open("#{@workingdir}/packer.json",'w') { |h|
     h.write json
     h.close
}
...
File.open("#{@workingdir}/vars.json", 'w' ) do |h|
  ...
  generate_vagrant_file(updated_json_hash, )
...
if __FILE__ == $0
  generator = PackerJsonGenerator.new
